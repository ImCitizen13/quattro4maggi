...
import ThreadsSpotlight from "./ThreadsSpotlight";
...
export default function ThreadsView() {
  ...
  const revealFromEnd = useSharedValue(false);
    ...
    (current, previous) => {
      ...
      if (current === "pulling") {
        ...
        beadHead.value = withTiming(
          ...
          {
            duration:
              Math.abs(beadHead.value - beadTarget.value) * BEAD_LEG_MS,
  ...
  const dim = useDerivedValue(() => {
    ...
    return interpolate(progress.value, [0, DIM_BY], [0, 1], Extrapolation.CLAMP);
  });
  ...
  const canvasWidth = width - ICON_SLOT * 2;
  const headerAnimatedStyle = useAnimatedStyle(() => {
    return {height: HEADER_HEIGHT + (progress.value * 20)
    }//
  })
  return (
    <Animated.View style={[styles.header, headerAnimatedStyle]}>
      ...
      <Canvas style={{ width: canvasWidth, height: HEADER_HEIGHT }}>
        <ThreadsSpotlight
          width={canvasWidth}

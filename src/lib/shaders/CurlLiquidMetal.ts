/**
 * Curl Liquid Metal Shader (SkSL)
 *
 * A differentiated liquid metal shader using curl noise for swirling,
 * fluid-like distortion patterns.
 *
 * DIFFERENTIATORS from original paper-design shader:
 * - Uses curl noise (divergence-free) for organic swirling motion
 * - Creates more fluid, turbulent patterns than Perlin/simplex
 * - Customizable metal colors via uniforms (gold, copper, rose gold, etc.)
 * - Optimized for mobile/React Native
 *
 * CURL NOISE:
 * Curl noise is computed by taking the curl of a potential field.
 * In 2D: curl(f) = (∂f/∂y, -∂f/∂x)
 * This creates divergence-free vector fields that look like fluid flow.
 *
 * UNIFORMS:
...
export const curlLiquidMetalShader = `
...
// ============================================================================
// CURL NOISE (2D) - Divergence-free noise for fluid-like motion
// ============================================================================
...
// Base Perlin noise (used as potential field for curl)
float noiseBase(float2 p) {
  float2 i = floor(p);
  float2 f = fract(p);
...
/**
 * Curl noise in 2D
 * Computes the curl of a scalar potential field (noise)
 * curl(f) = (∂f/∂y, -∂f/∂x)
 * This creates a divergence-free (incompressible) vector field
 */
float2 curlNoise(float2 p) {
  const float eps = 0.001;

  // Compute partial derivatives using finite differences
  float n1 = noiseBase(p + float2(0.0, eps));
  float n2 = noiseBase(p - float2(0.0, eps));
  float dfdx_y = (n1 - n2) / (2.0 * eps);

  n1 = noiseBase(p + float2(eps, 0.0));
  n2 = noiseBase(p - float2(eps, 0.0));
  float dfdy_x = (n1 - n2) / (2.0 * eps);

  // Curl in 2D: (∂f/∂y, -∂f/∂x)
  return float2(dfdx_y, -dfdy_x);
}

/**
 * Fractal Brownian Motion curl noise
 * Layers multiple octaves for more complex swirling patterns
 */
float2 fbmCurl(float2 p, int octaves) {
  ...
  return curl / totalAmplitude;
}

/**
 * Scalar curl magnitude for distortion
 * Returns a single value based on curl field magnitude and direction
 */
float curlScalar(float2 p, float t) {
  // Add time-based animation to the curl field
  float2 animatedP = p + float2(t * 0.1, t * 0.15);

  // Get multi-octave curl
  float2 curl = fbmCurl(animatedP, 3);

  // Combine curl components into scalar for distortion
  // The swirling effect comes from mixing magnitude with directional bias
  float magnitude = length(curl);
  float angle = atan(curl.y, curl.x);

  // Create swirling scalar pattern
  return magnitude * sin(angle + p.x * 2.0 + p.y * 2.0);
}
...
half4 main(float2 fragCoord) {
  ...
  // Use custom metal colors from uniforms
  float3 color1 = iColorHighlight;
  float3 color2 = iColorShadow + float3(0.0, 0.0, 0.1 * smoothstep(0.7, 1.3, diagTLtoBR));

  // Apply curl-based UV distortion for swirling motion
  float2 curlOffset = fbmCurl(uv * 2.0 + t * 0.5, 2) * iDistortion * 0.15;

  float2 grad_uv = uv - 0.5 + curlOffset;
  ...
  // Use curl noise for swirling, fluid-like distortion
  float noise = curlScalar(uv * 3.0, t);
